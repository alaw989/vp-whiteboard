...
- **Next:** Goal items 1-5 are now covered: reconnect re-convergence (it.1), no local reset on reconnect (it.1), duplicate + out-of-order + stale-subset SYNC_FULL (it.1 implicitly + it.3 explicitly), and timer hygiene (it.2). Remaining candidate: assert a 4001 close DURING the reconnect backoff cancels the pending timer (today `shouldReconnectOnClose(4001)` is false so `scheduleReconnect` isn't called, but if a close lands while a previously-scheduled backoff timer is still pending, the old timer may still fire — worth locking or fixing). Could also exercise `sendFullStateSync`'s empty-doc early return, or a SYNC_FULL frame with a zero-length update (`applyRemoteSyncFrame` early-returns on `byteLength === 0`).
