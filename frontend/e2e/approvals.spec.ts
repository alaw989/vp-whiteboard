...
import { login, loginSubmit, seedPendingUser } from './helpers'
...
test.describe('Admin approvals', () => {
  ...
  test('pending user is blocked, admin approves, pending user can sign in', async ({ page }) => {
    ...
    await loginSubmit(page, E2E_PENDING_EMAIL, E2E_PENDING_PASSWORD)
    ...
    await login(page, { email: E2E_ADMIN_EMAIL, password: E2E_ADMIN_PASSWORD })
    ...
    await login(page, { email: E2E_PENDING_EMAIL, password: E2E_PENDING_PASSWORD })
  })
  ...
  test('admin deny removes the pending user and they cannot sign in', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /Sign Out/ }).click()
    await page.waitForURL(/\/login$/, { timeout: 15000 })
    await loginSubmit(page, E2E_DENY_EMAIL, E2E_DENY_PASSWORD)
    await expect(page).toHaveURL(/\/login$/)
