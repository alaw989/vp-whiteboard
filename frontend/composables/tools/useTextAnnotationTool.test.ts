...
describe('useTextAnnotationTool', () => {
  ...
  it('confirmAnnotation emits element with leader and text', () => {
    ...
    expect(emitted.data.text).toBe('Hello')
    expect(emitted.data.leaderLine).toEqual({ start: [300, 200], end: [100, 100] })
    expect(emitted.data.x).toBe(100)
    expect(emitted.data.y).toBe(100)
  })
