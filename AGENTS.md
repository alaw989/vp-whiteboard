...
1. **Local work** — branch off `develop` (e.g. `feat/...`, `fix/...`). Before pushing, run locally in `frontend/`:
   ...
   - `npm test` (must pass — currently 755 tests)
   - `npm run coverage` (must exit 0 — gates lines/stmts 82, branches 84.5, functions 86.5)
   - And at the repo root: `php artisan test` (must pass — currently 77 tests, no `.env` needed since `phpunit.xml` sets `APP_KEY`)
...
- `npm test` = `vitest run` (755 tests, 57 files). `npm run typecheck` = `vue-tsc --noEmit`. `npm run coverage` = `vitest run --coverage` gated on thresholds (lines/stmts 82, branches 84.5, functions 86.5); thresholds live in the script's CLI flags so `npm test` stays the fast green loop.
- Backend: `php artisan test` (77 tests). `phpunit.xml` sets `APP_KEY` + SQLite in-memory, so it runs without a `.env`. Coverage gate (73% statements) is enforced in CI via pcov + a clover-XML parse step — PHPUnit has no native min-coverage flag.
...
1. **Grid/snap + dimension units** — visual grid toggle, snapping, mm/in + scale calibration.
2. **Custom stamp upload** — image stamps beyond the fixed APPROVED/REVISED/NOTE/FOR REVIEW set.
3. **Admin user management** — user list + disable/reset for `is_admin` owners (beyond approve/deny).
4. **Offline editing (PWA)** — service worker + local-first edits synced on reconnect (reconnect/resume hardening already laid groundwork).
5. **Accessibility audit** — keyboard focus in the toolbar, focus-trapping in the text-annotation modal, aria on the mobile toolbar.
6. **Visual regression screenshot tests** — golden-image styling diffs the fingerprint-based tests can't catch.
7. **Promote `e2e` CI job to a required check** — once the e2e job (added in #68/#69) has stayed green across several PRs; may need to tame the 2 remaining warm-up flakes (approvals `waitForURL` 15s, collab WS-sync 20s poll under 2-worker CI load — both retry-pass).
8. (Ideas for later) — PDF layer rendering perf, viewport-clipping correctness on zoom, admin approval email test on staging with real SMTP, onboarding/empty-state UX.
3. **Custom stamp upload** — image stamps beyond the fixed APPROVED/REVISED/NOTE/FOR REVIEW set.
4. **Admin user management** — user list + disable/reset for `is_admin` owners (beyond approve/deny).
5. **Offline editing (PWA)** — service worker + local-first edits synced on reconnect (reconnect/resume hardening already laid groundwork).
6. **Accessibility audit** — keyboard focus in the toolbar, focus-trapping in the text-annotation modal, aria on the mobile toolbar.
7. **Visual regression screenshot tests** — golden-image styling diffs the fingerprint-based tests can't catch.
8. **Promote `e2e` CI job to a required check** — once the e2e job (added in #68/#69) has stayed green across several PRs; may need to tame the 2 remaining warm-up flakes (approvals `waitForURL` 15s, collab WS-sync 20s poll under 2-worker CI load — both retry-pass).
9. (Ideas for later) — PDF layer rendering perf, viewport-clipping correctness on zoom, admin approval email test on staging with real SMTP, onboarding/empty-state UX.
...
- Vector/SVG export — PDF export draws shapes as **vector primitives** (crisp at any zoom / print) on top of the raster PNG (which still renders image/document layers). New pure `frontend/utils/vectorExport.ts` (Konva-free, **100% branch**): stroke (point→path smoothing), line, rect, circle, ellipse, polyline, arc, fillet-arc, dimension, arrow, stamp, text, revision-cloud + unknown-type/degenerate guards. `useExport.ts` lays the vector layer after `addImage(raster)`; `[id].vue` passes elements. API/filenames/progress/toasts unchanged. Frontend 755 → 807; e2e export spec 3/3. **Loop halted after 1 stall (jsPDF ESM probe failed under Node — not a code issue)** then proceeded; branch rebased on develop pre-PR — #89, release #90.
...
- Board dashboard — search (`?search=`, LIKE wildcards escaped) + sort (`?sort=recent|alpha`) on `GET /api/whiteboards`; `archived_at` migration + `active()`/`archived()` scopes (archived hidden by default, `?include_archived=1` to view); owner-only `POST /api/whiteboards/{id}/archive` + `/unarchive`; frontend `index.vue` search input (debounced, stale-response guarded), Recent/Alphabetical sort control, archive/unarchive in the card menu + archived view, client-side canvas thumbnails (`WhiteboardThumbnail.vue` from `canvas_state.elements`, resize-aware, icon fallback — no server image storage). `utils/dashboard.ts` pure-logic layer (99.18% covered). Backend 58 → 77, frontend 690 → 744, e2e +4 (`dashboard.spec.ts`). **e2e caught 3 real product bugs mid-loop:** thumbnails never rendered (canvas `v-if` chicken-and-egg), `include_archived` returned everything, seed fixture `updated_at` silently dropped by mass-assignment guard — #79, sync #80, release #81.
- Save-state indicator — `useSaveState` composable (`saved | saving | offline` state machine with idle-timeout safety net so the badge never sticks in 'saving'; `markDirty`/`onSaveStart`/`onSaveSuccess`/`onSaveFailure` with `retryCount` + `lastFailedAt`, 100% covered / 11 tests) wired into `frontend/pages/whiteboard/[id].vue`: header badge (`data-testid="save-state-badge"`, `aria-live="polite"`) next to the Connection pill showing Saving…/Saved/Offline – retrying (N). The previously-swallowed PATCH error now surfaces as 'offline' and auto-retries on the next 30s tick — save cadence (30s interval + 2s debounce + keepalive flush) and PATCH body UNCHANGED, backend untouched. Frontend 744 → 755; e2e 71/71 zero flakes. **Loop halted after 1 iteration on an opencode billing error (insufficient balance), not a code issue** — iteration 1 shipped the complete feature; re-verified (typecheck/npm test 755/coverage/build/e2e 71) and shipped manually — #83, sync #84, release #85.

**Current health (Aug 13, 2026):** `npm run typecheck` clean, `npm test` 807/807 (60 files), `npm run coverage` green at thresholds (82/82/84.5/86.5), `npm run test:e2e` 68 passed (3 warm-up retry-flakes) / 0 failed, `php artisan test` 77 passed (78.56% statements ≥ CI gate 73). All branch-protection checks (`test`, `backend-test`) green. Local dev stack ports: Laravel :8002, Nuxt :3000, WS relay :3001. Droplets: staging+prod on `165.245.141.179` (relay :3003 staging / :3001 prod). e2e must run with Nuxt `TEST=1` (disables devtools overlay) and a clean stack — a stale Nuxt on :3000 makes a fresh one fall back to :3001 and collide with the WS relay.
