...
2. **Grid/snap + dimension units** — visual grid toggle, snapping, mm/in + scale calibration.
...
- Collab realtime hardening (Phase 1) — root-caused the recurring "realtime stopped working AGAIN" (prod relay was rejecting connections — 27,497 rejections to one board — until a deploy restart cleared it; same signature as #42/#49). Fixed a 3-part fragility chain: (1) relay cached NEGATIVE auth verdicts for 60s so one transient Laravel failure locked out every viewer of that credential for a minute → negatives now cache 5s, fetch-failures (status 0) not cached at all; (2) relay forwarded client `pong`s to peers instead of refreshing `lastPong`, so the server-side heartbeat never worked → pongs now refresh in-place (backgrounded tabs survive); (3) client permanently gave up on 4001 (one spurious rejection killed realtime until manual refresh) → `shouldReconnectOnClose(code, authRetries)` bounded to `MAX_AUTH_RETRIES=3` then gives up. Plus `visibilitychange` sends an immediate keepalive when a tab regains focus (browsers throttle the 25s interval in hidden tabs). Log hygiene: binary-frame logging rate-limited per connection (large syncs always logged, small deltas summarized once/2s) — the 4.9GB/72M-line pm2 out log; `pm2-logrotate` installed on the droplet (100M max, retain 5, compress, daily). 63 new tests (relay auth-cache TTLs, pong keepalive, bounded 4001 retry, visibility ping, log rate-limit). Frontend 755 → 818. Live-verified both directions on staging AND prod (rectangle/circle propagated, DB persisted, rate-limited log format confirmed) — #92, release #93.

**Current health (Aug 13, 2026):** `npm run typecheck` clean, `npm test` 818/818 (60 files), `npm run coverage` green at thresholds (82/82/84.5/86.5), `php artisan test` 77 passed (78.56% statements ≥ CI gate 73). All branch-protection checks (`test`, `backend-test`) green. Local dev stack ports: Laravel :8002, Nuxt :3000, WS relay :3001. Droplets: staging+prod on `165.245.141.179` (relay :3003 staging / :3001 prod). e2e must run with Nuxt `TEST=1` (disables devtools overlay) and a clean stack — a stale Nuxt on :3000 makes a fresh one fall back to :3001 and collide with the WS relay.
