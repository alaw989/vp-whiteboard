...
### 2. Data migration: Supabase → MySQL — RESOLVED: start fresh
**Decision (verified 2026-06-21): start with an empty DB — no migration needed.**
An audit of the production Supabase project (`qhoeiectyttqijabck`) found it holds
only **dev/test data**: 24 whiteboards (all named "New Whiteboard", created
2026-06-01→06-20) with **0 of 24 containing any drawing elements** (every
`canvas_state` empty), plus 7 test file uploads. There is no real client engineering
markup to preserve, and the app used anonymous handles (no Supabase auth users to
migrate). Production deploys with an empty MySQL DB; users register fresh Breeze
accounts. No data loss because there's no real data.

> **If real data ever exists later**, the migration is straightforward: Supabase
> `whiteboards` (`id, name, created_by, canvas_state` JSON, timestamps) and
> `whiteboard_files` (`id, whiteboard_id, file_name, file_type, storage_path,
> file_size, metadata`) map 1:1 onto the Laravel schema (`created_by` is a string,
> `share_token` is generated by `Whiteboard::creating`). Files live in Supabase
> Storage bucket `whiteboard-files` at `whiteboard_id/<ts>-<rand>.ext` (or as flat
> files in prod `public/uploads/` for local-fallback uploads) and would be copied
> into Laravel `storage/app/public/`. `canvas_state` carries no file URLs (files
> render via a separate document layer keyed by `fileId`), so no URL remapping is
> needed. A reusable `php artisan` migration command was deferred since there was
> nothing to migrate.
